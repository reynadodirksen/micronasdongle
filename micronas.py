...
class USBProgrammer:
    ...
    def read_voltage_out(self):
        ...
        voltage = ((total / samples) * 0.995107*(config.vsup)/1000)-0.01#unsure about the 4.91 
    ...
    def read_continuous_voltage(self):
        ...
        return round(((total / samples) * 0.995107*(config.vsup)/1000)-0.01, 3)#(((((total / samples) * 4.91) / 1000)*1.01044932079)-0.01, 3)
    ...
    def write_setup(self, setup):
        ...
        value = 0x0300
        crc = self._calc_crc(((0x300 | 0x01C | 0x002) << 16) | value, 26)
        data = '{:05X}'.format((value << 4) + (crc & 0x0F))
